...
if [ "$cm" == "0" ]; then
	mapping=$(/bin/cat /lib/usb-scanner/standard_layout.conf || ( /etc/usb-scanner/log-out "2" "/etc/usb-scanner/xbox-driver.sh" '/lib/usb-scanner/standard_layout.conf cannot be read' "usb-scanner" "$PWD" "$0" && exit 2 ))
elif [ "$cm" == "1" ]; then
	mapping=$(/etc/usb-scanner/import /lib/usb-scanner/usb-scanner.lib parser)
	contents=$(/bin/cat $HOME/.xboxdrv/layout.conf)
else
	/etc/usb-scanner/log-out "2" "/etc/usb-scanner/xbox-driver.sh" "Error reading flags." "usb-scanner" "$PWD" "$0"
	exit 2
fi
eval "$mapping" || ( /etc/usb-scanner/log-out "2" "/etc/usb-scanner/xbox-driver.sh" "Cannot parse designated conf file." "usb-scanner" "$PWD" "$0" ; exit 2 )
if [ "$fm" == "0" ]; then
	inf=1
	while [ "$inf" == "1" ]; do
	#scan usb devices and get list of Product:Vendor IDs (PVIDs)
		/bin/sleep 0.1s
		#gain VID/PID
		usb=$(/etc/usb-scanner/poll.sh) || /etc/usb-scanner/log-out "2" "/etc/usb-scanner/xbox-driver.sh" "poll.sh has failed. Please see error log for more details." "usb-scanner" "$PWD" "$0" 
		/bin/rm $usb_loc
		#current USB state
		...
		done
		{
			...
			for y in $usb; do
				/bin/sleep 0.1s
				{
					...
					/etc/usb-scanner/log-out "2" "/etc/usb-scanner/xbox-driver.sh" "Joystick not recognized. VPID: $y" "usb-scanner" "$PWD" "$0"
				}
			done
		} || {
			#clear it's sort of cache on the internal drive
